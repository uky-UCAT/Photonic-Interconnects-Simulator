...
SC_MODULE(TGatewayInterface) {
	...
	//Photonic offline delay calculation

	/** Ports for the bottom electrical layer - END **/

	/** Ports for optical communication  - START **/

	sc_out<TFlit> mwmr_data_tx[DEFAULT_MWMR_WAVEGUIDES][4];
	sc_in<TFlit> mwmr_data_rx[DEFAULT_MWMR_WAVEGUIDES][4];
	...
	TSrcDstPhotonicDelay* getDelayObj;

	int cycle_count;
	int MAX_PHOTONIC_DELAY;
	...
	void delayCount();
	void Optical_rxProcess();   // The receive Process from optical layer
	void txProcess();           // The Tx Process
	void mwmr_tx_data();         // Send the data to crossbar
	...
	//ack_txc_arbiter(sc_gen_unique_name("arbiter_out"))
	{
		...
		getDelayObj = new TSrcDstPhotonicDelay("newtable.csv");
		MAX_PHOTONIC_DELAY = getDelayObj->getMaxPhotonicDelay();
       // cout<<"Max Photonic Delay"<<MAX_PHOTONIC_DELAY<<endl;
...
private:
	...
	double previous_transfer_photonic[DEFAULT_MWMR_WAVEGUIDES][4];
