...
int sc_main(int arg_num, char* arg_vet[]) {
    ...
    // Signals
    sc_clock clock("clock", 0.2 , SC_NS);
    sc_clock photonic_clock("photonic_clock",60, SC_NS);

    sc_signal<bool> reset;
    //Offline Photonic Delay Instance
    //TSrcDstPhotonicDelay* getDelayObj = new TSrcDstPhotonicDelay("offlineoutputfile.csv");
    //cout<< getDelayObj->getPhotonicDelay("0","33")<<endl;
    // Benchmark Instance
    benchmark* b_mark;
    //string benchmark_type = arg_vet[1];
    //cout<<"args read: "<< arg_vet[1]<< "benchmark type: "<< benchmark_type << endl;
    b_mark = new benchmark("benchmark",arg_vet);
    b_mark->clock(clock);
    b_mark->reset(reset);
    // NoC instance
    TNoC* n = new TNoC("NoC", *b_mark );
